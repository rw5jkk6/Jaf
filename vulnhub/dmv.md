...
- webサイトを見るが適当に入力するが送信がpostなのでBurpSuiteを起動する
  ...
  - (追記)リクエストメッセージは同じBurpsuiteのChromeを使っているので、formatはme and mygirlと同じになる
- osコマンドインジェクションができるかを確認する
  ...
  - postのボディ部分のyt_url=のところに`` --exec%3c`ls${IFS}-la` ``を入力するとResponseにコマンドの結果であるディレクトリの一覧がパーミッションと一緒に返ってくる
    - `${IFS}`はbashでのスペース、`%3c`はURLエンコーダのスペース。普通のスペースではエラーになる。`+`もスペースの代わりに使うことがある(domdom)が、ここではエラーになる
...
- rootになる
  - しばらく待つとrootになる
  - rootにプロンプトがついているのは、リバースシェルの際に`bash -i`がついているため。ただし、リバースシェルなので対話型シェルではないので、対話型シェルにする

### 補足
- Burpsuiteを使わずにcurlでreverse-shellを実行する
  - ブラウザでサイトを開く、inspectからnetworkの画面を開く
  - サイトに適当にunkoとでも入力して、convertボタンを押す。すると、postというのが出てくる。そこをクリックすると、今送ったリクエストを見ることができる。ここで右クリックして`Copy value`を選択して`Copy as cURL`を押す
  - 次に、memo.txtを開けてコピーしたのを貼り付ける。そして postのボディ部分のyt_url=のところに`` --exec%3c`bash${IFS}file.sh` ``を書き換える。
  - いつも通りにncで待ち受けてから、memo.txtをコピーしてターミナルに貼り付けて実行するとreverse-shellが成功する

## Question
- `@{IFS}`はリクエストでなぜ使った
- echoでシングルクォートとバッククォートの違いは
- BurpのRepeaterはどんな機能
- www-dataになる際に、対話型シェルをしなかったのにプロンプトついてたのはなぜ
- pspyをやってみる
