...
- reverse-shellをbase64に変換させてする 
  ...
  - 実行する
- www-dataで侵入
  - 対話型シェルっぽいプロンプトが返ってきたが確かめてみる。これはリバースシェルのコードに`bash -i ~`がついているため
  - `tty`
...
- samのプロンプトをつける
  -  `bash`するとシェルになる
- (追記)`sudo -u sam`でユーザ切り替えすると正規のユーザ切り替えではないため、正規のユーザ切り替えすると`.bashrc`が起動時に呼び込まれるが、これが今回読み込まれていない。だから`$`だけになる。ただし、これでもコマンドは使える。`.bashrc`が読まれていないのは`ll`コマンドを打ってみたらわかるが、エラーになる。bashコマンドで`ll`を打つと反応が返ってくるのが、bashrcが呼ばれている証拠
-
- id 特になし
- sudo -l
  - /root/lago
