...
- サイトを見る
  ...
  - webサイトでcrack stationにさっきの暗号を貼るとhostingerというのがわかる

- ftpから情報を得る
  - hostinger:hostinger 
  - hint.txtがある 
- ターミナルでhint.txtを分析する
  - 暗号に=がついているのでbase64をdecodeする。初めのは3回する、２つ目は1回する。すると、ヒントが出てくる。
  - `http://cryptii.com/pipes/vigenere-cipher`にアクセスする。
  - 設定は次のようにする
    - Decodeにする
...
- (1)サイトの`/panel/`のログイン画面からログインする
  - ユーザ名は`dora`,passwordは`E7~`で始まるの
  - ダッシュボードにログインする
...
- (2)(追記) exploitdbでpythonのコード探す
  ...
  - `python exploit.py -u http://venom.box/panel/ -l dora -p E7~`
...
- sudo -l
  - (root) ALL, !/bin/su
  - これは/bin/su以外ならなんでも使ってrootになれるって意味
- `sudo /bin/bash -i` 
