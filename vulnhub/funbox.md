...
- nmap
  - 21,22,80,33060
- サイトを見る
  - サイトが見れない。そしてfunbox.fritz.boxにリダイレクトされているのがわかる。inspectのnetworkを見てもう一度更新する
  - 2回通信されているのがわかる。1回目のGETをクリックすると右側に新しい表示が出てくる。ヘッダのresponseを見ると、`Location: http://funbox.fritz.box/`というのがある。これがリダイレクトさせている原因
  - そして２つ目のGETで、新しいrequestを送っているが、名前解決できないのでresponseも返ってきていない  
  - nmapにも出てきてたが、名前解決ができるように/etc/hostsを書き換えないとサイト見れない
- サイトを見る
  - ソースは何もない
- gobusterする
  - /secret/
