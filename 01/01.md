...
- `file=log_03.txt`を以下のように書き直す。これはリクエストを利用して、ディレクトリリバーサルをしている
  - `file=../etc/passwd`と書き直してForwardボタンを押す。ユーザのファイルは出てこないので`../`を増やしていく。
  - `file=../`のところでマウスの右クリックして`send to repeater`を選択する。すると上のrepeaterが赤色みたいになるので、そこを押す。
  - ここで`file=../../etc/passwd`の`../`順に増やして左上のsendボタンを押して右側のResponseの反応を見る
  - 何度か繰り返していくと/etc/passwdが見れる
- 表示をコピーする
...
- (追記)john the ripperに渡すファイルの中身
  - `webadmin:$1$webadmin$3sXBx~:1001:1001:webadmin,,,:/home/webadmin:/bin/bash`
  - 上のは/etc/passwdの、一行を抜き出したものだが、コロンで区切ってあって左からユーザ名、パスワード、ユーザ番号といろいろ書いてある
  - コロンで区切った左二つがjohn the ripperに読み込まれる。ただし、コロンが一つの場合はパスワードのみを読み込むようになっている
...
```
### root権限の取得(その3,PwnKit)
- gccはないが、バイナリのファイルを送ることでできる
...
```
  ...
  - 書いてあるかを確認
  - crontab -l
  - `service cron start` または `systemctl start cron`
  - service cron status

## (補足) curlでディレクトリトラバーサルをしてpasswdを見る(２通り)
- (1)`curl -X POST -b "pass=~" -d 'file=../../../../../etc/passwd' "http://192.168.56.101/admin/dashboard.php?.page=log"`
  - `-X` httpメソッドの指定
  - `-b` クッキーの指定
  - `-d` postリクエストのフォームを送信する
- (2)ブラウザのinspectから右クリックでcopy value -> copy as curl をメモに貼り付けてcurlする
